...
model = Sequential()
model.add(LSTM(32, input_shape=(timesteps,data_dim)))  # returns a sequence of vectors of dimension 32
	...
	for i in range(1):
		...
		if(delayVideoToData<=0):	
			...
			while (fc<max_size and ret):
				...
				if (fc>=59 and fc<max_size-60):
					RNN_train_data = np.array(RNN_train_data_current[l[1]*(fc*30-1770):(l[1]*((fc+1)*30))-1])
					RNN_train_data = np.reshape(RNN_train_data,(1,1800,l[1]))
			...
			while (fc<(max_size) and ret):
				...
				while(fc1<30):
					ret, buf[fc1] = cap.read()
					x = image.img_to_array(np.round(resize(buf[fc1],(1,224,224,3))))	
					x = np.expand_dims(x, axis=0)
					x = preprocess_input(x)
					features = model1.predict(x)
					l = np.shape(features)
				...
				if (fc>=59 and fc<max_size-60):
					...
					RNN_train_data = np.reshape(RNN_train_data,(1,1800,l[1]))
					RNN_train_data = RNN_train_data/255	
					model.fit(RNN_train_data,np.reshape(semantic[fc,:],(1,3)),batch_size=1,epochs=1)
...
if(delayVideoToData<=0):	
	...
	while (fc<max_size and ret):
		...
		if (fc>=59 and fc<max_size-60):
			RNN_test_data = np.array(RNN_train_data_current[l[1]*(fc*30-1770):(l[1]*((fc+1)*30))-1])
			RNN_test_data = np.reshape(RNN_test_data,(1,1800,l[1]))
			RNN_test_data = RNN_test_data/255	
			score = model.evaluate(RNN_test_data,np.reshape(semantic[fc+int(delayVideoToData),:],(1,3)),batch_size=1)
			print(score)
	...
	while (fc<(max_size) and ret):
		...
		if (fc>=59 and fc<max_size-60):
			...
			RNN_test_data = np.reshape(RNN_test_data,(1,1800,l[1]))
			RNN_test_data = RNN_test_data/255	
			score = model.evaluate(RNN_test_data,np.reshape(semantic[fc,:],(1,3)),batch_size=1)
			print(score)
cap.release()